...
if app == 'Epidemic':
  ...
  COMPUTE_PERIOD = 4
if testbed == 'orbit':
    from nodesmap_orbit import nodesinv
else:
    from nodesmap_clust import nodesinv
...
def parse_link_events ():
  ...
  for line in file:
    ...
    if words[1] == 'link':
      ...
      node = int (words[2])
      nbr = int(nodesinv[words[3]])-1
      if node != nbr:
        if words[4] == '+link':
          add = True
          add_event (time, (LINK, node, add))
        elif words[4] =='-link':
          add = False
          add_event (time, (LINK, node, add))
    elif words[1] == 'ip':
...
def parse_trace_files ():
  ...
  for filename in os.listdir (os.path.join (sim_dir, 'pcaps')):
    if filename.endswith ('.pcap.trace'):
      ...
      node_ip = filename.split ('_')[1].split('-')[0]
      node_id = nodesinv[node_ip]-1
      #print file, node_ip
      ...
      for line in file:
        ...
        if [ip, node_ip] in apptable [interval]:
