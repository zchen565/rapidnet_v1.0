 ...
 */
  ...
  */
    ...
    {
      return rtuple(APPTABLE_NAME, attrlist(
                        ...
                        Ptr<RapidNetApplicationBase> dst, string id)
    {
      ...
      src->Inject(rtuple("eQuery", attr("eQuery_attr1", Ipv4Value, from),
                         attr("eQuery_attr2", Ipv4Value, to),
                         attr("eQuery_attr3", StrValue, id)));
      dst->Inject(rtuple("eQuery_Dst", attr("eQuery_attr1", Ipv4Value, from),
                         attr("eQuery_attr2", Ipv4Value, to),
                         attr("eQuery_attr3", StrValue, id)));
    }
    ...
    ClearNodeStates(ApplicationContainer &apps)
    {
      for (uint32_t i = 0; i < apps.GetN(); ++i)
      {
        Ptr<RapidNetApplicationBase> app = Ptr<RapidNetApplicationBase>(
            dynamic_cast<RapidNetApplicationBase *>(PeekPointer(apps.Get(i))));
        app->Inject(rtuple("clear_node_state"));
      }
    }
 ...
 */
                ...
                Ptr<RapidNetApplicationBase> landmarkApp)
    {
      ...
      Ptr<Tuple> landmarkTuple = rtuple(chord::Chord::LANDMARK,
 ...
 */
    ...
    {
      Ipv4Address localAddr = app->GetAddress();
      Ptr<Tuple> predTuple = rtuple(chord::Chord::PRED,
 ...
 */
    ...
    {
      Ipv4Address localAddr = app->GetAddress();
      Ptr<Tuple> nffTuple = rtuple(chord::Chord::NEXTFINGERFIX,
    ...
    InstallDecoratorTriggers(string appName, ApplicationContainer &apps)
    {
      ...
      else if (appName == "SimLsPeriodic" || appName == "SimLsTriggered" || appName == "SimHslsPeriodic" || appName == "SimHslsTriggered")
      {
