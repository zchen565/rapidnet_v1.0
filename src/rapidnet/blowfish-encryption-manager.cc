...
void BlowfishEncryptionManager::SetPrincipal(
...
BlowfishEncryptionManager::OpenSslSeal(Ptr<Tuple> tuple, string keyId)
{
  ...
  {
    NS_LOG_ERROR(FormatMsg("Error while EVP_SealInit", " for tuple " << tuple));
    return Tuple::NewEmpty();
  }

  if (!EVP_SealUpdate(ctx, cipher, &outlen, message, msgLen))
  {
    NS_LOG_ERROR(FormatMsg("Error while EVP_SealUpdate", " for tuple " << tuple));
    return Tuple::NewEmpty();
  }

  if (!EVP_SealFinal(ctx, cipher + outlen, &tmplen))
  {
    NS_LOG_ERROR(FormatMsg("Error while EVP_SealFinal", " for tuple " << tuple));
    return Tuple::NewEmpty();
  }
  ...
  {
    NS_LOG_ERROR(FormatMsg("Buffer overflow error", " for tuple " << tuple));
    return Tuple::NewEmpty();
  }
