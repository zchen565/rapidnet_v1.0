...
// rt ans(@Local, Z) :- candidate(@Local, Z).
//mapping hash value
r7 shaResult(@Local, VID, Content) :- word(@Local, W), VID := f_sha1("word"+Local+W), Name := "word", Content := Name+"_"+W.
r8 shaResult(@Local, VID, Content) :- hasImg(@Local, X, Rln, Y), VID := f_sha1("hasImg"+Local+X+Rln+Y), Name := "hasImg", Content := Name+"_"+X+"_"+Rln+"_"+Y.
r9 shaResult(@Local, VID, Content) :- hasQ(@Local, X, Rln, Y), VID := f_sha1("hasQ"+Local+X+Rln+Y), Name := "hasQ", Content := Name+"_"+X+"_"+Rln+"_"+Y.
r10 shaResult(@Local, VID, Content) :- sim(@Local, X, Y), VID := f_sha1("sim"+Local+X+Y), Name := "sim", Content := Name+"_"+X+"_"+Y.
r11 shaResult(@Local, VID, Content) :- simRep1(@Local, X, Y), VID := f_sha1("simRep1"+Local+X+Y), Name := "simRep1", Content := Name+"_"+X+"_"+Y.
r12 shaResult(@Local, VID, Content) :- simRep2(@Local, X, Y), VID := f_sha1("simRep2"+Local+X+Y), Name := "simRep2", Content := Name+"_"+X+"_"+Y.
r13 shaResult(@Local, VID, Content) :- ans(@Local, Z), VID := f_sha1("ans"+Local+Z), Name := "ans", Content := Name+"_"+Z.
r14 shaResult(@Local, VID, Content) :- candidate(@Local, Z), VID := f_sha1("candidate"+Local+Z), Name := "candidate", Content := Name+"_"+Z.
r15 shaResult(@Local, VID, Content) :- hasImgAns(@Local, Z, X, Rln, Y), VID := f_sha1("hasImgAns"+Local+Z+X+Rln+Y), Name := "hasImgAns", Content := Name+"_"+Z+"_"+X+"_"+Rln+"_"+Y.
